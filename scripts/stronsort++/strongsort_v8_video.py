...
import time
video_path = "../tracking_course/test_videos/test_video1/5473765-uhd_3840_2160_24fps.mp4"
# video_path = "../tracking_course/test_videos/test_video1/PETS09-S2L2-raw.webm"
video_capture = cv2.VideoCapture(video_path)
frame_width = int(video_capture.get(cv2.CAP_PROP_FRAME_WIDTH))
frame_height = int(video_capture.get(cv2.CAP_PROP_FRAME_HEIGHT))
out = cv2.VideoWriter('output_video_strong_car_3.mp4', cv2.VideoWriter_fourcc(*'mp4v'), 15, (frame_width, frame_height))
while video_capture.isOpened():
    ...
    results = model.predict(frame, classes=[2,0,7])
    ...
    res = strongsort_list[0].update(xywhs, confs, clss, frame)
    if len(res)>0:
     boxes_track = res[:, :4]
     boxes_ids = res[:, 4].astype(int)
    
     frame = draw_bounding_boxes(frame, boxes_track, boxes_ids, frame_rate=15)  # Assuming 15 FPS for video
